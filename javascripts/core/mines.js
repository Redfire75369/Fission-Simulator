...
function updateMines() {
	for (let tier = 0; tier < min(player.meteor + 4, 8); tier++) {
		...
		document.getElementById(elements[tier] + "Mine").innerText = notation(player.mine.amount[tier]) + " (" + player.mine.bought[tier] + ")";
		document.getElementById(elements[tier] + "BuySingle").innerText = "Cost: " + notation(getMineCost(tier));
		document.getElementById(elements[tier] + "BuySingle").className = canBuyMine(tier) ? "minebtnbuy" : "minebtnlocked";
		document.getElementById(elements[tier] + "BuyMax").className = canBuyMine(tier) ? "minebtnbuy" : "minebtnlocked";
